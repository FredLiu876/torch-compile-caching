...
# kernel path: ./local_cache/as/casz3ep5dxrodxni63f6fnstjrnur6lm6uexbggylreroa3mnfgf.py
...
# kernel path: ./local_cache/gq/cgqx3nmcorekcfg75puyvmcqlwiyzlhx3t3zihkm6kjwgyyvcate.py
...
# kernel path: ./local_cache/er/cersrk2nyko3iqe53ublhxzd7kvjnpyiclim4njr3ujp6rdd2k2p.py
...
# kernel path: ./local_cache/km/ckmv7vyqc2f53thukszhxg5gz2b4utxx2dwjtx6xesswx5kk56go.py
...
# kernel path: ./local_cache/6e/c6ei2bl53v7ktkdwoeyac7wqnyjeifztupi5kngexaylzuyahjp2.py
...
# kernel path: ./local_cache/pa/cpax42nevszgrs2tm2wf6ksr7ss223bl62m5d3gfl6dipirhwljp.py
...
#   %constant_pad_nd : [num_users=2] = call_function[target=torch.ops.aten.constant_pad_nd.default](args = (%where, [0, %sub_108], 0.0), kwargs = {})
...
# kernel path: ./local_cache/5k/c5khdvdmkophlyqmatpbonjtmvckgp6kh4tu2ucyewdkyzkjokji.py
...
#   %mul_409 : [num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%primals_12, %convert_element_type_15), kwargs = {})
...
# kernel path: ./local_cache/f6/cf6xjdxdw44yjdxfqeatnhnrp7fqtoyoryf4okunqdopryxev4sc.py
...
# kernel path: ./local_cache/g3/cg3n6hvtx4do75kalztgulpaqncj5atzscma6f3lugjffmi36kyt.py
...
#   %mul_486 : [num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%primals_16, %convert_element_type_25), kwargs = {})
...
# kernel path: ./local_cache/5c/c5c3dyzot3z75ffv2rgpknn5id3oqk635hti77j23bckhoi55elb.py
...
#   %mul_795 : [num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%primals_21, %convert_element_type_35), kwargs = {})
...
def call(args):
    ...
    with torch.cuda._DeviceGuard(0):
        ...
        ps1 = 8 + s0 + (-1)*(s0 % 8)
        buf12 = empty_strided_cuda((1, 1, s0, 8 + s0 + (-1)*(s0 % 8)), (s0*s0 + 8*s0 + (-1)*s0*(s0 % 8), s0*s0 + 8*s0 + (-1)*s0*(s0 % 8), 8 + s0 + (-1)*(s0 % 8), 1), torch.float16)
        # Topologically Sorted Source Nodes: [attn_output], Original ATen: [aten.scalar_tensor, aten.where, aten.constant_pad_nd]
        triton_poi_fused_constant_pad_nd_scalar_tensor_where_5_xnumel = s0*s0 + 8*s0 + (-1)*s0*(s0 % 8)
        stream0 = get_raw_stream(0)
        triton_poi_fused_constant_pad_nd_scalar_tensor_where_5.run(primals_5, buf12, ps1, s0, s1, triton_poi_fused_constant_pad_nd_scalar_tensor_where_5_xnumel, stream=stream0)
        del primals_5
...
def benchmark_compiled_module(times=10, repeat=10):
    ...
    primals_1 = 11
    primals_2 = rand_strided((1, 11), (11, 1), device='cuda:0', dtype=torch.int64)
    primals_3 = rand_strided((128256, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_4 = 11
    primals_5 = rand_strided((1, 11), (11, 1), device='cuda:0', dtype=torch.int64)
