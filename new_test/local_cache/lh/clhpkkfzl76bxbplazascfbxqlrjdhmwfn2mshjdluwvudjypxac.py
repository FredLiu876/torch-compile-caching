...
# kernel path: ./local_cache/3k/c3kp3uisolqkxpkp44yxtd4m4gm5tvzfgnb4pfz7r37cv7lkmwh5.py
...
#   %mul_100 : [num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%primals_7, %convert_element_type_5), kwargs = {})
...
# kernel path: ./local_cache/33/c33xz4fpvgv2t4nx7tr2pu2g3vnnq7ulyz5nlfs5t2ncvsxw6rxc.py
...
# kernel path: ./local_cache/qp/cqpsarify7zeusikjixpajqdipwbvtpp2go4p4nwgovgccmgcsc3.py
...
# kernel path: ./local_cache/uo/cuomh3ofgkomnlkmhjmkvzwen2jxiis44ak5lub2pyxmj5x2lmhp.py
...
# kernel path: ./local_cache/x4/cx4jrcr7srhfpegblhxm3eej7eho6mlukfwcvg7yliegbq5shesd.py
...
# kernel path: ./local_cache/bi/cbi73qafz4gvj757kgkspff5kwwdua2pb7minxgo4jni7rbdprx2.py
...
triton_poi_fused_scalar_tensor_where_5 = async_compile.triton('triton_poi_fused_scalar_tensor_where_5', '''
import triton
import triton.language as tl
...
@triton_heuristics.pointwise(
    ...
    inductor_meta={'grid_type': 'Grid1D', 'autotune_hints': set(), 'kernel_name': 'triton_poi_fused_scalar_tensor_where_5', 'mutated_arg_names': [], 'optimize_mem': False, 'no_x_dim': False, 'num_load': 1, 'num_reduction': 0, 'backend_hash': '6BE828ED5586B6FAE8FF446A7636CA33B7485F906B06FE7C269A644474AB4AF1', 'are_deterministic_algorithms_enabled': False, 'assert_indirect_indexing': True, 'autotune_local_cache': True, 'autotune_pointwise': True, 'autotune_remote_cache': None, 'force_disable_caches': False, 'dynamic_scale_rblock': True, 'max_autotune': True, 'max_autotune_pointwise': False, 'min_split_scan_rblock': 256, 'spill_threshold': 16, 'store_cubin': False, 'coordinate_descent_tuning': True, 'coordinate_descent_search_radius': 1, 'coordinate_descent_check_all_directions': False},
    min_elem_per_thread=0
)
@triton.jit
def triton_poi_fused_scalar_tensor_where_5(in_ptr0, out_ptr0, ks0, ks1, xnumel, XBLOCK : tl.constexpr):
    xoffset = tl.program_id(0) * XBLOCK
    xindex = xoffset + tl.arange(0, XBLOCK)[:]
    xmask = xindex < xnumel
    x0 = (xindex % ks0)
    x1 = xindex // ks0
    x2 = xindex
    tl.device_assert((x0 < ks1) | ~(xmask), "index out of bounds: x0 < ks1")
...
# kernel path: ./local_cache/ft/cftrsow5qcbxopniidi22t6ajsrwmzbq4o64dytrbucrl2gj2i5m.py
...
#   hidden_states_8 => mul_401
#   pow_2 => pow_2
#   rsqrt_1 => rsqrt_1
#   to_7 => convert_element_type_15
#   variance_1 => mean_1
# Graph fragment:
#   %add_317 : [num_users=2] = call_function[target=torch.ops.aten.add.Tensor](args = (%embedding, %view_22), kwargs = {})
#   %convert_element_type_14 : [num_users=2] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%add_317, torch.float32), kwargs = {})
...
#   %add_330 : [num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%mean_1, 1e-05), kwargs = {})
#   %rsqrt_1 : [num_users=2] = call_function[target=torch.ops.aten.rsqrt.default](args = (%add_330,), kwargs = {})
...
#   %mul_401 : [num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%primals_12, %convert_element_type_15), kwargs = {})
...
def triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_6(in_out_ptr0, in_ptr0, in_ptr1, in_ptr2, out_ptr0, xnumel, r0_numel, XBLOCK : tl.constexpr, R0_BLOCK : tl.constexpr):
    ...
    for r0_offset in range(0, r0_numel, R0_BLOCK):
        ...
        tmp14 = tl.load(in_ptr0 + (r0_1 + 4096*x0), xmask & r0_mask, eviction_policy='evict_first', other=0.0).to(tl.float32)
        tmp15 = tl.load(in_ptr1 + (r0_1 + 4096*x0), xmask & r0_mask, eviction_policy='evict_first', other=0.0).to(tl.float32)
        tmp16 = tmp14 + tmp15
        tmp17 = tmp16.to(tl.float32)
        tmp18 = tmp17 * tmp12
        tmp19 = tmp18.to(tl.float32)
        tmp20 = tmp13 * tmp19
        tl.store(out_ptr0 + (r0_1 + 4096*x0), tmp20, xmask & r0_mask)
''', device_str='cuda')


# kernel path: ./local_cache/yf/cyfokqimwdos23k644loots6y52qecw7pxfxlxfpkicijq4ryanz.py
# Topologically Sorted Source Nodes: [silu, mul_10], Original ATen: [aten.silu, aten.mul]
# Source node to ATen node mapping:
#   mul_10 => mul_435
#   silu => convert_element_type_18, convert_element_type_19, mul_419, sigmoid
# Graph fragment:
...
#   %mul_419 : [num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%convert_element_type_18, %sigmoid), kwargs = {})
#   %convert_element_type_19 : [num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%mul_419, torch.float16), kwargs = {})
#   %mul_435 : [num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%convert_element_type_19, %view_26), kwargs = {})
...
# kernel path: ./local_cache/77/c77oi4n34co2lyl6w3uyrpu25nlsmz2qimbxcpjl3nsdatstbw7t.py
...
#   hidden_states_11 => mul_471
#   hidden_states_12 => mul_478
#   hidden_states_5 => add_317
#   hidden_states_9 => add_382
#   pow_3 => pow_3
#   rsqrt_2 => rsqrt_2
...
# Graph fragment:
#   %add_317 : [num_users=2] = call_function[target=torch.ops.aten.add.Tensor](args = (%embedding, %view_22), kwargs = {})
#   %add_382 : [num_users=3] = call_function[target=torch.ops.aten.add.Tensor](args = (%add_317, %view_28), kwargs = {})
#   %convert_element_type_24 : [num_users=2] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%add_382, torch.float32), kwargs = {})
...
#   %add_395 : [num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%mean_2, 1e-05), kwargs = {})
#   %rsqrt_2 : [num_users=2] = call_function[target=torch.ops.aten.rsqrt.default](args = (%add_395,), kwargs = {})
#   %mul_471 : [num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%convert_element_type_24, %rsqrt_2), kwargs = {})
#   %convert_element_type_25 : [num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%mul_471, torch.float16), kwargs = {})
#   %mul_478 : [num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%primals_16, %convert_element_type_25), kwargs = {})
...
def triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_8(in_out_ptr0, in_out_ptr1, in_ptr0, in_ptr1, in_ptr2, out_ptr0, xnumel, r0_numel, XBLOCK : tl.constexpr, R0_BLOCK : tl.constexpr):
    ...
    for r0_offset in range(0, r0_numel, R0_BLOCK):
        ...
        rindex = r0_index
        r0_1 = r0_index
        tmp0 = tl.load(in_ptr0 + (r0_1 + 4096*x0), xmask & r0_mask, eviction_policy='evict_first', other=0.0).to(tl.float32)
        tmp1 = tl.load(in_ptr1 + (r0_1 + 4096*x0), xmask & r0_mask, eviction_policy='evict_first', other=0.0).to(tl.float32)
        tmp3 = tl.load(in_out_ptr0 + (r0_1 + 4096*x0), xmask & r0_mask, eviction_policy='evict_first', other=0.0).to(tl.float32)
        tmp2 = tmp0 + tmp1
        tmp4 = tmp2 + tmp3
        tmp5 = tmp4.to(tl.float32)
        tmp6 = tmp5 * tmp5
        ...
        tl.store(in_out_ptr0 + (r0_1 + 4096*x0), tmp4, xmask & r0_mask)
    ...
    for r0_offset in range(0, r0_numel, R0_BLOCK):
        ...
        tmp16 = tl.load(in_out_ptr0 + (r0_1 + 4096*x0), xmask & r0_mask, eviction_policy='evict_first', other=0.0).to(tl.float32)
        tmp17 = tmp16.to(tl.float32)
        tmp18 = tmp17 * tmp14
        tmp19 = tmp18.to(tl.float32)
        tmp20 = tmp15 * tmp19
...
# kernel path: ./local_cache/ly/clyhrh4d6xaqs2dxaqhwkhrgdrw7d6mkpbykg5u5uxw52daopsjs.py
...
# Source node to ATen node mapping:
#   add_14 => add_627
#   hidden_states_15 => add_614
#   hidden_states_16 => convert_element_type_34
#   hidden_states_17 => mul_772
#   hidden_states_18 => mul_779
#   pow_4 => pow_4
#   rsqrt_3 => rsqrt_3
#   to_11 => convert_element_type_35
#   variance_3 => mean_3
# Graph fragment:
#   %add_614 : [num_users=3] = call_function[target=torch.ops.aten.add.Tensor](args = (%add_382, %view_42), kwargs = {})
#   %convert_element_type_34 : [num_users=2] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%add_614, torch.float32), kwargs = {})
...
#   %add_627 : [num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%mean_3, 1e-05), kwargs = {})
#   %rsqrt_3 : [num_users=2] = call_function[target=torch.ops.aten.rsqrt.default](args = (%add_627,), kwargs = {})
#   %mul_772 : [num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%convert_element_type_34, %rsqrt_3), kwargs = {})
#   %convert_element_type_35 : [num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%mul_772, torch.float16), kwargs = {})
#   %mul_779 : [num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%primals_21, %convert_element_type_35), kwargs = {})
...
def triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9(in_out_ptr0, in_out_ptr1, in_ptr0, in_ptr1, out_ptr0, xnumel, r0_numel, XBLOCK : tl.constexpr, R0_BLOCK : tl.constexpr):
    ...
    for r0_offset in range(0, r0_numel, R0_BLOCK):
        ...
        r0_1 = r0_index
        tmp0 = tl.load(in_ptr0 + (r0_1 + 4096*x0), xmask & r0_mask, eviction_policy='evict_first', other=0.0).to(tl.float32)
        tmp1 = tl.load(in_out_ptr0 + (r0_1 + 4096*x0), xmask & r0_mask, eviction_policy='evict_first', other=0.0).to(tl.float32)
        tmp2 = tmp0 + tmp1
        tmp3 = tmp2.to(tl.float32)
        tmp4 = tmp3 * tmp3
        tmp5 = tl.broadcast_to(tmp4, [XBLOCK, R0_BLOCK])
        tmp7 = _tmp6 + tmp5
        _tmp6 = tl.where(r0_mask & xmask, tmp7, _tmp6)
        tl.store(in_out_ptr0 + (r0_1 + 4096*x0), tmp2, xmask & r0_mask)
    tmp6 = tl.sum(_tmp6, 1)[:, None]
    tmp8 = 4096.0
    tmp9 = (tmp6 / tmp8)
    tmp10 = 1e-05
    ...
    for r0_offset in range(0, r0_numel, R0_BLOCK):
        ...
        tmp17 = tmp16.to(tl.float32)
        tmp18 = tmp13 * tmp17
        tl.store(out_ptr0 + (r0_1 + 4096*x0), tmp18, xmask & r0_mask)
''', device_str='cuda')
...
def call(args):
    ...
    assert_size_stride(primals_5, (1, s1), (s1, 1))
    ...
    assert_size_stride(primals_10, (1024, 4096), (4096, 1))
    assert_size_stride(primals_11, (4096, 4096), (4096, 1))
    assert_size_stride(primals_12, (4096, ), (1, ))
    assert_size_stride(primals_13, (14336, 4096), (4096, 1))
    assert_size_stride(primals_14, (14336, 4096), (4096, 1))
    assert_size_stride(primals_15, (4096, 14336), (14336, 1))
    ...
    assert_size_stride(primals_19, (1024, 4096), (4096, 1))
    ...
    assert_size_stride(primals_23, (14336, 4096), (4096, 1))
    assert_size_stride(primals_24, (4096, 14336), (14336, 1))
    ...
    assert_size_stride(primals_28, (1024, 4096), (4096, 1))
    ...
    assert_size_stride(primals_32, (14336, 4096), (4096, 1))
    assert_size_stride(primals_33, (4096, 14336), (14336, 1))
    ...
    assert_size_stride(primals_41, (14336, 4096), (4096, 1))
    assert_size_stride(primals_42, (4096, 14336), (14336, 1))
    ...
    assert_size_stride(primals_46, (1024, 4096), (4096, 1))
    ...
    assert_size_stride(primals_50, (14336, 4096), (4096, 1))
    assert_size_stride(primals_51, (4096, 14336), (14336, 1))
    ...
    assert_size_stride(primals_55, (1024, 4096), (4096, 1))
    ...
    assert_size_stride(primals_59, (14336, 4096), (4096, 1))
    assert_size_stride(primals_60, (4096, 14336), (14336, 1))
    ...
    assert_size_stride(primals_68, (14336, 4096), (4096, 1))
    assert_size_stride(primals_69, (4096, 14336), (14336, 1))
    assert_size_stride(primals_70, (4096, ), (1, ))
    assert_size_stride(primals_71, (4096, 4096), (4096, 1))
    assert_size_stride(primals_72, (1024, 4096), (4096, 1))
    assert_size_stride(primals_73, (1024, 4096), (4096, 1))
    ...
    assert_size_stride(primals_77, (14336, 4096), (4096, 1))
    assert_size_stride(primals_78, (4096, 14336), (14336, 1))
    assert_size_stride(primals_79, (4096, ), (1, ))
    assert_size_stride(primals_80, (4096, 4096), (4096, 1))
    assert_size_stride(primals_81, (1024, 4096), (4096, 1))
    assert_size_stride(primals_82, (1024, 4096), (4096, 1))
    assert_size_stride(primals_83, (4096, 4096), (4096, 1))
    assert_size_stride(primals_84, (4096, ), (1, ))
    assert_size_stride(primals_85, (14336, 4096), (4096, 1))
    assert_size_stride(primals_86, (14336, 4096), (4096, 1))
    assert_size_stride(primals_87, (4096, 14336), (14336, 1))
    ...
    assert_size_stride(primals_91, (1024, 4096), (4096, 1))
    assert_size_stride(primals_92, (4096, 4096), (4096, 1))
    assert_size_stride(primals_93, (4096, ), (1, ))
    assert_size_stride(primals_94, (14336, 4096), (4096, 1))
    assert_size_stride(primals_95, (14336, 4096), (4096, 1))
    assert_size_stride(primals_96, (4096, 14336), (14336, 1))
    ...
    assert_size_stride(primals_100, (1024, 4096), (4096, 1))
    ...
    assert_size_stride(primals_104, (14336, 4096), (4096, 1))
    assert_size_stride(primals_105, (4096, 14336), (14336, 1))
    ...
    assert_size_stride(primals_109, (1024, 4096), (4096, 1))
    ...
    assert_size_stride(primals_113, (14336, 4096), (4096, 1))
    assert_size_stride(primals_114, (4096, 14336), (14336, 1))
    ...
    assert_size_stride(primals_118, (1024, 4096), (4096, 1))
    assert_size_stride(primals_119, (4096, 4096), (4096, 1))
    assert_size_stride(primals_120, (4096, ), (1, ))
    assert_size_stride(primals_121, (14336, 4096), (4096, 1))
    assert_size_stride(primals_122, (14336, 4096), (4096, 1))
    assert_size_stride(primals_123, (4096, 14336), (14336, 1))
    ...
    assert_size_stride(primals_127, (1024, 4096), (4096, 1))
    assert_size_stride(primals_128, (4096, 4096), (4096, 1))
    assert_size_stride(primals_129, (4096, ), (1, ))
    assert_size_stride(primals_130, (14336, 4096), (4096, 1))
    assert_size_stride(primals_131, (14336, 4096), (4096, 1))
    assert_size_stride(primals_132, (4096, 14336), (14336, 1))
    ...
    assert_size_stride(primals_136, (1024, 4096), (4096, 1))
    ...
    assert_size_stride(primals_140, (14336, 4096), (4096, 1))
    assert_size_stride(primals_141, (4096, 14336), (14336, 1))
    assert_size_stride(primals_142, (4096, ), (1, ))
    assert_size_stride(primals_143, (4096, 4096), (4096, 1))
    assert_size_stride(primals_144, (1024, 4096), (4096, 1))
    assert_size_stride(primals_145, (1024, 4096), (4096, 1))
    ...
    assert_size_stride(primals_149, (14336, 4096), (4096, 1))
    assert_size_stride(primals_150, (4096, 14336), (14336, 1))
    assert_size_stride(primals_151, (4096, ), (1, ))
    assert_size_stride(primals_152, (4096, 4096), (4096, 1))
    assert_size_stride(primals_153, (1024, 4096), (4096, 1))
    assert_size_stride(primals_154, (1024, 4096), (4096, 1))
    ...
    assert_size_stride(primals_158, (14336, 4096), (4096, 1))
    assert_size_stride(primals_159, (4096, 14336), (14336, 1))
    ...
    assert_size_stride(primals_163, (1024, 4096), (4096, 1))
    ...
    assert_size_stride(primals_167, (14336, 4096), (4096, 1))
    assert_size_stride(primals_168, (4096, 14336), (14336, 1))
    ...
    assert_size_stride(primals_172, (1024, 4096), (4096, 1))
    ...
    assert_size_stride(primals_176, (14336, 4096), (4096, 1))
    assert_size_stride(primals_177, (4096, 14336), (14336, 1))
    assert_size_stride(primals_178, (4096, ), (1, ))
    assert_size_stride(primals_179, (4096, 4096), (4096, 1))
    assert_size_stride(primals_180, (1024, 4096), (4096, 1))
    assert_size_stride(primals_181, (1024, 4096), (4096, 1))
    ...
    assert_size_stride(primals_185, (14336, 4096), (4096, 1))
    assert_size_stride(primals_186, (4096, 14336), (14336, 1))
    assert_size_stride(primals_187, (4096, ), (1, ))
    assert_size_stride(primals_188, (4096, 4096), (4096, 1))
    assert_size_stride(primals_189, (1024, 4096), (4096, 1))
    assert_size_stride(primals_190, (1024, 4096), (4096, 1))
    assert_size_stride(primals_191, (4096, 4096), (4096, 1))
    assert_size_stride(primals_192, (4096, ), (1, ))
    assert_size_stride(primals_193, (14336, 4096), (4096, 1))
    assert_size_stride(primals_194, (14336, 4096), (4096, 1))
    assert_size_stride(primals_195, (4096, 14336), (14336, 1))
    ...
    assert_size_stride(primals_199, (1024, 4096), (4096, 1))
    assert_size_stride(primals_200, (4096, 4096), (4096, 1))
    assert_size_stride(primals_201, (4096, ), (1, ))
    assert_size_stride(primals_202, (14336, 4096), (4096, 1))
    assert_size_stride(primals_203, (14336, 4096), (4096, 1))
    assert_size_stride(primals_204, (4096, 14336), (14336, 1))
    ...
    assert_size_stride(primals_208, (1024, 4096), (4096, 1))
    ...
    assert_size_stride(primals_212, (14336, 4096), (4096, 1))
    assert_size_stride(primals_213, (4096, 14336), (14336, 1))
    ...
    assert_size_stride(primals_217, (1024, 4096), (4096, 1))
    ...
    assert_size_stride(primals_221, (14336, 4096), (4096, 1))
    assert_size_stride(primals_222, (4096, 14336), (14336, 1))
    ...
    assert_size_stride(primals_230, (14336, 4096), (4096, 1))
    assert_size_stride(primals_231, (4096, 14336), (14336, 1))
    ...
    assert_size_stride(primals_235, (1024, 4096), (4096, 1))
    ...
    assert_size_stride(primals_239, (14336, 4096), (4096, 1))
    assert_size_stride(primals_240, (4096, 14336), (14336, 1))
    ...
    assert_size_stride(primals_244, (1024, 4096), (4096, 1))
    ...
    assert_size_stride(primals_248, (14336, 4096), (4096, 1))
    assert_size_stride(primals_249, (4096, 14336), (14336, 1))
    assert_size_stride(primals_250, (4096, ), (1, ))
    assert_size_stride(primals_251, (4096, 4096), (4096, 1))
    assert_size_stride(primals_252, (1024, 4096), (4096, 1))
    assert_size_stride(primals_253, (1024, 4096), (4096, 1))
    ...
    assert_size_stride(primals_257, (14336, 4096), (4096, 1))
    assert_size_stride(primals_258, (4096, 14336), (14336, 1))
    assert_size_stride(primals_259, (4096, ), (1, ))
    assert_size_stride(primals_260, (4096, 4096), (4096, 1))
    assert_size_stride(primals_261, (1024, 4096), (4096, 1))
    assert_size_stride(primals_262, (1024, 4096), (4096, 1))
    assert_size_stride(primals_263, (4096, 4096), (4096, 1))
    assert_size_stride(primals_264, (4096, ), (1, ))
    assert_size_stride(primals_265, (14336, 4096), (4096, 1))
    assert_size_stride(primals_266, (14336, 4096), (4096, 1))
    assert_size_stride(primals_267, (4096, 14336), (14336, 1))
    ...
    assert_size_stride(primals_271, (1024, 4096), (4096, 1))
    assert_size_stride(primals_272, (4096, 4096), (4096, 1))
    assert_size_stride(primals_273, (4096, ), (1, ))
    assert_size_stride(primals_274, (14336, 4096), (4096, 1))
    assert_size_stride(primals_275, (14336, 4096), (4096, 1))
    assert_size_stride(primals_276, (4096, 14336), (14336, 1))
    ...
    assert_size_stride(primals_280, (1024, 4096), (4096, 1))
    ...
    assert_size_stride(primals_284, (14336, 4096), (4096, 1))
    assert_size_stride(primals_285, (4096, 14336), (14336, 1))
    assert_size_stride(primals_286, (4096, ), (1, ))
    assert_size_stride(primals_287, (4096, 4096), (4096, 1))
    assert_size_stride(primals_288, (1024, 4096), (4096, 1))
    assert_size_stride(primals_289, (1024, 4096), (4096, 1))
    ...
    assert_size_stride(primals_293, (14336, 4096), (4096, 1))
    assert_size_stride(primals_294, (4096, 14336), (14336, 1))
    assert_size_stride(primals_295, (4096, ), (1, ))
    assert_size_stride(primals_296, (128256, 4096), (4096, 1))
    with torch.cuda._DeviceGuard(0):
        ...
        triton_red_fused__to_copy_add_embedding_mean_mul_pow_rsqrt_0.run(buf4, primals_2, primals_3, primals_7, buf0, buf5, s0, 4096, stream=stream0)
        del primals_3
        ...
        extern_kernels.bmm(reinterpret_tensor(primals_6, (1, 64, 1), (0, 1, 0), 0), buf1, out=buf2)
        del primals_6
        buf6 = empty_strided_cuda((s0, 4096), (4096, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf5, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_8, (4096, 4096), (1, 4096), 0), out=buf6)
        buf7 = empty_strided_cuda((s0, 1024), (1024, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_1], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf5, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_9, (4096, 1024), (1, 4096), 0), out=buf7)
        buf8 = empty_strided_cuda((s0, 1024), (1024, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_2], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf5, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_10, (4096, 1024), (1, 4096), 0), out=buf8)
        ps0 = 512*s0
        ...
        buf12 = empty_strided_cuda((1, 1, s0, s0), (s0*s0, s0*s0, s0, 1), torch.float16)
        # Topologically Sorted Source Nodes: [attn_output], Original ATen: [aten.scalar_tensor, aten.where]
        triton_poi_fused_scalar_tensor_where_5_xnumel = s0*s0
        stream0 = get_raw_stream(0)
        triton_poi_fused_scalar_tensor_where_5.run(primals_5, buf12, s0, s1, triton_poi_fused_scalar_tensor_where_5_xnumel, stream=stream0)
        del primals_5
        # Topologically Sorted Source Nodes: [attn_output], Original ATen: [aten._scaled_dot_product_efficient_attention]
        buf13 = torch.ops.aten._scaled_dot_product_efficient_attention.default(buf11, reinterpret_tensor(buf9, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf10, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf12, (1, 32, s0, s0), (s0*s0, 0, s0, 1), 0), True, scale=0.08838834764831845)
        buf14 = buf13[0]
        ...
        extern_kernels.mm(reinterpret_tensor(buf14, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_11, (4096, 4096), (1, 4096), 0), out=buf18)
        ...
        # Topologically Sorted Source Nodes: [hidden_states_5, hidden_states_6, pow_2, variance_1, add_8, rsqrt_1, hidden_states_7, to_7, hidden_states_8], Original ATen: [aten.add, aten._to_copy, aten.pow, aten.mean, aten.rsqrt, aten.mul]
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_6.run(buf20, buf0, buf18, primals_12, buf21, s0, 4096, stream=stream0)
        buf22 = empty_strided_cuda((s0, 14336), (14336, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_4], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf21, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_13, (4096, 14336), (1, 4096), 0), out=buf22)
        buf23 = empty_strided_cuda((s0, 14336), (14336, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_5], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf21, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_14, (4096, 14336), (1, 4096), 0), out=buf23)
        ...
        extern_kernels.mm(reinterpret_tensor(buf24, (s0, 14336), (14336, 1), 0), reinterpret_tensor(primals_15, (14336, 4096), (1, 14336), 0), out=buf25)
        ...
        # Topologically Sorted Source Nodes: [hidden_states_5, hidden_states_9, hidden_states_10, pow_3, variance_2, add_10, rsqrt_2, hidden_states_11, to_9, hidden_states_12], Original ATen: [aten.add, aten._to_copy, aten.pow, aten.mean, aten.rsqrt, aten.mul]
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_8.run(buf26, buf28, buf0, buf18, primals_16, buf29, s0, 4096, stream=stream0)
        buf30 = empty_strided_cuda((s0, 4096), (4096, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_7], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf29, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_17, (4096, 4096), (1, 4096), 0), out=buf30)
        buf31 = buf8; del buf8  # reuse
        # Topologically Sorted Source Nodes: [linear_8], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf29, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_18, (4096, 1024), (1, 4096), 0), out=buf31)
        buf32 = buf7; del buf7  # reuse
        # Topologically Sorted Source Nodes: [linear_9], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf29, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_19, (4096, 1024), (1, 4096), 0), out=buf32)
        ...
        buf36 = torch.ops.aten._scaled_dot_product_efficient_attention.default(buf35, reinterpret_tensor(buf33, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf34, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf12, (1, 32, s0, s0), (s0*s0, 0, s0, 1), 0), True, scale=0.08838834764831845)
        buf37 = buf36[0]
        ...
        extern_kernels.mm(reinterpret_tensor(buf37, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_20, (4096, 4096), (1, 4096), 0), out=buf41)
        ...
        # Topologically Sorted Source Nodes: [hidden_states_15, hidden_states_16, pow_4, variance_3, add_14, rsqrt_3, hidden_states_17, to_11, hidden_states_18], Original ATen: [aten.add, aten._to_copy, aten.pow, aten.mean, aten.rsqrt, aten.mul]
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf42, buf44, buf26, primals_21, buf45, s0, 4096, stream=stream0)
        buf46 = empty_strided_cuda((s0, 14336), (14336, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_11], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf45, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_22, (4096, 14336), (1, 4096), 0), out=buf46)
        buf47 = empty_strided_cuda((s0, 14336), (14336, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_12], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf45, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_23, (4096, 14336), (1, 4096), 0), out=buf47)
        ...
        extern_kernels.mm(reinterpret_tensor(buf48, (s0, 14336), (14336, 1), 0), reinterpret_tensor(primals_24, (14336, 4096), (1, 14336), 0), out=buf49)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf50, buf52, buf42, primals_25, buf53, s0, 4096, stream=stream0)
        buf54 = empty_strided_cuda((s0, 4096), (4096, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_14], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf53, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_26, (4096, 4096), (1, 4096), 0), out=buf54)
        buf55 = buf32; del buf32  # reuse
        # Topologically Sorted Source Nodes: [linear_15], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf53, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_27, (4096, 1024), (1, 4096), 0), out=buf55)
        buf56 = buf31; del buf31  # reuse
        # Topologically Sorted Source Nodes: [linear_16], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf53, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_28, (4096, 1024), (1, 4096), 0), out=buf56)
        ...
        buf60 = torch.ops.aten._scaled_dot_product_efficient_attention.default(buf59, reinterpret_tensor(buf57, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf58, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf12, (1, 32, s0, s0), (s0*s0, 0, s0, 1), 0), True, scale=0.08838834764831845)
        buf61 = buf60[0]
        ...
        extern_kernels.mm(reinterpret_tensor(buf61, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_29, (4096, 4096), (1, 4096), 0), out=buf65)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf66, buf68, buf50, primals_30, buf69, s0, 4096, stream=stream0)
        buf70 = empty_strided_cuda((s0, 14336), (14336, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_18], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf69, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_31, (4096, 14336), (1, 4096), 0), out=buf70)
        buf71 = empty_strided_cuda((s0, 14336), (14336, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_19], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf69, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_32, (4096, 14336), (1, 4096), 0), out=buf71)
        ...
        extern_kernels.mm(reinterpret_tensor(buf72, (s0, 14336), (14336, 1), 0), reinterpret_tensor(primals_33, (14336, 4096), (1, 14336), 0), out=buf73)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf74, buf76, buf66, primals_34, buf77, s0, 4096, stream=stream0)
        buf78 = empty_strided_cuda((s0, 4096), (4096, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_21], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf77, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_35, (4096, 4096), (1, 4096), 0), out=buf78)
        buf79 = buf56; del buf56  # reuse
        # Topologically Sorted Source Nodes: [linear_22], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf77, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_36, (4096, 1024), (1, 4096), 0), out=buf79)
        buf80 = buf55; del buf55  # reuse
        # Topologically Sorted Source Nodes: [linear_23], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf77, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_37, (4096, 1024), (1, 4096), 0), out=buf80)
        ...
        buf84 = torch.ops.aten._scaled_dot_product_efficient_attention.default(buf83, reinterpret_tensor(buf81, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf82, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf12, (1, 32, s0, s0), (s0*s0, 0, s0, 1), 0), True, scale=0.08838834764831845)
        buf85 = buf84[0]
        ...
        extern_kernels.mm(reinterpret_tensor(buf85, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_38, (4096, 4096), (1, 4096), 0), out=buf89)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf90, buf92, buf74, primals_39, buf93, s0, 4096, stream=stream0)
        buf94 = empty_strided_cuda((s0, 14336), (14336, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_25], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf93, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_40, (4096, 14336), (1, 4096), 0), out=buf94)
        buf95 = empty_strided_cuda((s0, 14336), (14336, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_26], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf93, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_41, (4096, 14336), (1, 4096), 0), out=buf95)
        ...
        extern_kernels.mm(reinterpret_tensor(buf96, (s0, 14336), (14336, 1), 0), reinterpret_tensor(primals_42, (14336, 4096), (1, 14336), 0), out=buf97)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf98, buf100, buf90, primals_43, buf101, s0, 4096, stream=stream0)
        buf102 = empty_strided_cuda((s0, 4096), (4096, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_28], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf101, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_44, (4096, 4096), (1, 4096), 0), out=buf102)
        buf103 = buf80; del buf80  # reuse
        # Topologically Sorted Source Nodes: [linear_29], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf101, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_45, (4096, 1024), (1, 4096), 0), out=buf103)
        buf104 = buf79; del buf79  # reuse
        # Topologically Sorted Source Nodes: [linear_30], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf101, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_46, (4096, 1024), (1, 4096), 0), out=buf104)
        ...
        buf108 = torch.ops.aten._scaled_dot_product_efficient_attention.default(buf107, reinterpret_tensor(buf105, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf106, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf12, (1, 32, s0, s0), (s0*s0, 0, s0, 1), 0), True, scale=0.08838834764831845)
        buf109 = buf108[0]
        ...
        extern_kernels.mm(reinterpret_tensor(buf109, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_47, (4096, 4096), (1, 4096), 0), out=buf113)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf114, buf116, buf98, primals_48, buf117, s0, 4096, stream=stream0)
        ...
        extern_kernels.mm(reinterpret_tensor(buf117, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_49, (4096, 14336), (1, 4096), 0), out=buf118)
        ...
        extern_kernels.mm(reinterpret_tensor(buf117, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_50, (4096, 14336), (1, 4096), 0), out=buf119)
        ...
        extern_kernels.mm(reinterpret_tensor(buf120, (s0, 14336), (14336, 1), 0), reinterpret_tensor(primals_51, (14336, 4096), (1, 14336), 0), out=buf121)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf122, buf124, buf114, primals_52, buf125, s0, 4096, stream=stream0)
        buf126 = empty_strided_cuda((s0, 4096), (4096, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_35], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf125, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_53, (4096, 4096), (1, 4096), 0), out=buf126)
        buf127 = buf104; del buf104  # reuse
        # Topologically Sorted Source Nodes: [linear_36], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf125, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_54, (4096, 1024), (1, 4096), 0), out=buf127)
        buf128 = buf103; del buf103  # reuse
        # Topologically Sorted Source Nodes: [linear_37], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf125, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_55, (4096, 1024), (1, 4096), 0), out=buf128)
        ...
        buf132 = torch.ops.aten._scaled_dot_product_efficient_attention.default(buf131, reinterpret_tensor(buf129, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf130, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf12, (1, 32, s0, s0), (s0*s0, 0, s0, 1), 0), True, scale=0.08838834764831845)
        buf133 = buf132[0]
        ...
        extern_kernels.mm(reinterpret_tensor(buf133, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_56, (4096, 4096), (1, 4096), 0), out=buf137)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf138, buf140, buf122, primals_57, buf141, s0, 4096, stream=stream0)
        ...
        extern_kernels.mm(reinterpret_tensor(buf141, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_58, (4096, 14336), (1, 4096), 0), out=buf142)
        ...
        extern_kernels.mm(reinterpret_tensor(buf141, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_59, (4096, 14336), (1, 4096), 0), out=buf143)
        ...
        extern_kernels.mm(reinterpret_tensor(buf144, (s0, 14336), (14336, 1), 0), reinterpret_tensor(primals_60, (14336, 4096), (1, 14336), 0), out=buf145)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf146, buf148, buf138, primals_61, buf149, s0, 4096, stream=stream0)
        buf150 = empty_strided_cuda((s0, 4096), (4096, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_42], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf149, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_62, (4096, 4096), (1, 4096), 0), out=buf150)
        buf151 = buf128; del buf128  # reuse
        # Topologically Sorted Source Nodes: [linear_43], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf149, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_63, (4096, 1024), (1, 4096), 0), out=buf151)
        buf152 = buf127; del buf127  # reuse
        # Topologically Sorted Source Nodes: [linear_44], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf149, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_64, (4096, 1024), (1, 4096), 0), out=buf152)
        ...
        buf156 = torch.ops.aten._scaled_dot_product_efficient_attention.default(buf155, reinterpret_tensor(buf153, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf154, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf12, (1, 32, s0, s0), (s0*s0, 0, s0, 1), 0), True, scale=0.08838834764831845)
        buf157 = buf156[0]
        ...
        extern_kernels.mm(reinterpret_tensor(buf157, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_65, (4096, 4096), (1, 4096), 0), out=buf161)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf162, buf164, buf146, primals_66, buf165, s0, 4096, stream=stream0)
        ...
        extern_kernels.mm(reinterpret_tensor(buf165, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_67, (4096, 14336), (1, 4096), 0), out=buf166)
        ...
        extern_kernels.mm(reinterpret_tensor(buf165, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_68, (4096, 14336), (1, 4096), 0), out=buf167)
        ...
        extern_kernels.mm(reinterpret_tensor(buf168, (s0, 14336), (14336, 1), 0), reinterpret_tensor(primals_69, (14336, 4096), (1, 14336), 0), out=buf169)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf170, buf172, buf162, primals_70, buf173, s0, 4096, stream=stream0)
        buf174 = empty_strided_cuda((s0, 4096), (4096, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_49], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf173, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_71, (4096, 4096), (1, 4096), 0), out=buf174)
        buf175 = buf152; del buf152  # reuse
        # Topologically Sorted Source Nodes: [linear_50], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf173, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_72, (4096, 1024), (1, 4096), 0), out=buf175)
        buf176 = buf151; del buf151  # reuse
        # Topologically Sorted Source Nodes: [linear_51], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf173, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_73, (4096, 1024), (1, 4096), 0), out=buf176)
        ...
        buf180 = torch.ops.aten._scaled_dot_product_efficient_attention.default(buf179, reinterpret_tensor(buf177, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf178, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf12, (1, 32, s0, s0), (s0*s0, 0, s0, 1), 0), True, scale=0.08838834764831845)
        buf181 = buf180[0]
        ...
        extern_kernels.mm(reinterpret_tensor(buf181, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_74, (4096, 4096), (1, 4096), 0), out=buf185)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf186, buf188, buf170, primals_75, buf189, s0, 4096, stream=stream0)
        ...
        extern_kernels.mm(reinterpret_tensor(buf189, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_76, (4096, 14336), (1, 4096), 0), out=buf190)
        ...
        extern_kernels.mm(reinterpret_tensor(buf189, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_77, (4096, 14336), (1, 4096), 0), out=buf191)
        ...
        extern_kernels.mm(reinterpret_tensor(buf192, (s0, 14336), (14336, 1), 0), reinterpret_tensor(primals_78, (14336, 4096), (1, 14336), 0), out=buf193)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf194, buf196, buf186, primals_79, buf197, s0, 4096, stream=stream0)
        buf198 = empty_strided_cuda((s0, 4096), (4096, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_56], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf197, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_80, (4096, 4096), (1, 4096), 0), out=buf198)
        buf199 = buf176; del buf176  # reuse
        # Topologically Sorted Source Nodes: [linear_57], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf197, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_81, (4096, 1024), (1, 4096), 0), out=buf199)
        buf200 = buf175; del buf175  # reuse
        # Topologically Sorted Source Nodes: [linear_58], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf197, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_82, (4096, 1024), (1, 4096), 0), out=buf200)
        ...
        buf204 = torch.ops.aten._scaled_dot_product_efficient_attention.default(buf203, reinterpret_tensor(buf201, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf202, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf12, (1, 32, s0, s0), (s0*s0, 0, s0, 1), 0), True, scale=0.08838834764831845)
        buf205 = buf204[0]
        ...
        extern_kernels.mm(reinterpret_tensor(buf205, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_83, (4096, 4096), (1, 4096), 0), out=buf209)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf210, buf212, buf194, primals_84, buf213, s0, 4096, stream=stream0)
        ...
        extern_kernels.mm(reinterpret_tensor(buf213, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_85, (4096, 14336), (1, 4096), 0), out=buf214)
        ...
        extern_kernels.mm(reinterpret_tensor(buf213, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_86, (4096, 14336), (1, 4096), 0), out=buf215)
        ...
        extern_kernels.mm(reinterpret_tensor(buf216, (s0, 14336), (14336, 1), 0), reinterpret_tensor(primals_87, (14336, 4096), (1, 14336), 0), out=buf217)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf218, buf220, buf210, primals_88, buf221, s0, 4096, stream=stream0)
        buf222 = empty_strided_cuda((s0, 4096), (4096, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_63], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf221, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_89, (4096, 4096), (1, 4096), 0), out=buf222)
        buf223 = buf200; del buf200  # reuse
        # Topologically Sorted Source Nodes: [linear_64], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf221, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_90, (4096, 1024), (1, 4096), 0), out=buf223)
        buf224 = buf199; del buf199  # reuse
        # Topologically Sorted Source Nodes: [linear_65], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf221, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_91, (4096, 1024), (1, 4096), 0), out=buf224)
        ...
        buf228 = torch.ops.aten._scaled_dot_product_efficient_attention.default(buf227, reinterpret_tensor(buf225, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf226, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf12, (1, 32, s0, s0), (s0*s0, 0, s0, 1), 0), True, scale=0.08838834764831845)
        buf229 = buf228[0]
        ...
        extern_kernels.mm(reinterpret_tensor(buf229, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_92, (4096, 4096), (1, 4096), 0), out=buf233)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf234, buf236, buf218, primals_93, buf237, s0, 4096, stream=stream0)
        ...
        extern_kernels.mm(reinterpret_tensor(buf237, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_94, (4096, 14336), (1, 4096), 0), out=buf238)
        ...
        extern_kernels.mm(reinterpret_tensor(buf237, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_95, (4096, 14336), (1, 4096), 0), out=buf239)
        ...
        extern_kernels.mm(reinterpret_tensor(buf240, (s0, 14336), (14336, 1), 0), reinterpret_tensor(primals_96, (14336, 4096), (1, 14336), 0), out=buf241)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf242, buf244, buf234, primals_97, buf245, s0, 4096, stream=stream0)
        buf246 = empty_strided_cuda((s0, 4096), (4096, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_70], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf245, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_98, (4096, 4096), (1, 4096), 0), out=buf246)
        buf247 = buf224; del buf224  # reuse
        # Topologically Sorted Source Nodes: [linear_71], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf245, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_99, (4096, 1024), (1, 4096), 0), out=buf247)
        buf248 = buf223; del buf223  # reuse
        # Topologically Sorted Source Nodes: [linear_72], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf245, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_100, (4096, 1024), (1, 4096), 0), out=buf248)
        ...
        buf252 = torch.ops.aten._scaled_dot_product_efficient_attention.default(buf251, reinterpret_tensor(buf249, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf250, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf12, (1, 32, s0, s0), (s0*s0, 0, s0, 1), 0), True, scale=0.08838834764831845)
        buf253 = buf252[0]
        ...
        extern_kernels.mm(reinterpret_tensor(buf253, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_101, (4096, 4096), (1, 4096), 0), out=buf257)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf258, buf260, buf242, primals_102, buf261, s0, 4096, stream=stream0)
        ...
        extern_kernels.mm(reinterpret_tensor(buf261, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_103, (4096, 14336), (1, 4096), 0), out=buf262)
        ...
        extern_kernels.mm(reinterpret_tensor(buf261, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_104, (4096, 14336), (1, 4096), 0), out=buf263)
        ...
        extern_kernels.mm(reinterpret_tensor(buf264, (s0, 14336), (14336, 1), 0), reinterpret_tensor(primals_105, (14336, 4096), (1, 14336), 0), out=buf265)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf266, buf268, buf258, primals_106, buf269, s0, 4096, stream=stream0)
        buf270 = empty_strided_cuda((s0, 4096), (4096, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_77], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf269, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_107, (4096, 4096), (1, 4096), 0), out=buf270)
        buf271 = buf248; del buf248  # reuse
        # Topologically Sorted Source Nodes: [linear_78], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf269, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_108, (4096, 1024), (1, 4096), 0), out=buf271)
        buf272 = buf247; del buf247  # reuse
        # Topologically Sorted Source Nodes: [linear_79], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf269, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_109, (4096, 1024), (1, 4096), 0), out=buf272)
        ...
        buf276 = torch.ops.aten._scaled_dot_product_efficient_attention.default(buf275, reinterpret_tensor(buf273, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf274, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf12, (1, 32, s0, s0), (s0*s0, 0, s0, 1), 0), True, scale=0.08838834764831845)
        buf277 = buf276[0]
        ...
        extern_kernels.mm(reinterpret_tensor(buf277, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_110, (4096, 4096), (1, 4096), 0), out=buf281)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf282, buf284, buf266, primals_111, buf285, s0, 4096, stream=stream0)
        ...
        extern_kernels.mm(reinterpret_tensor(buf285, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_112, (4096, 14336), (1, 4096), 0), out=buf286)
        ...
        extern_kernels.mm(reinterpret_tensor(buf285, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_113, (4096, 14336), (1, 4096), 0), out=buf287)
        ...
        extern_kernels.mm(reinterpret_tensor(buf288, (s0, 14336), (14336, 1), 0), reinterpret_tensor(primals_114, (14336, 4096), (1, 14336), 0), out=buf289)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf290, buf292, buf282, primals_115, buf293, s0, 4096, stream=stream0)
        buf294 = empty_strided_cuda((s0, 4096), (4096, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_84], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf293, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_116, (4096, 4096), (1, 4096), 0), out=buf294)
        buf295 = buf272; del buf272  # reuse
        # Topologically Sorted Source Nodes: [linear_85], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf293, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_117, (4096, 1024), (1, 4096), 0), out=buf295)
        buf296 = buf271; del buf271  # reuse
        # Topologically Sorted Source Nodes: [linear_86], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf293, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_118, (4096, 1024), (1, 4096), 0), out=buf296)
        ...
        buf300 = torch.ops.aten._scaled_dot_product_efficient_attention.default(buf299, reinterpret_tensor(buf297, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf298, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf12, (1, 32, s0, s0), (s0*s0, 0, s0, 1), 0), True, scale=0.08838834764831845)
        buf301 = buf300[0]
        ...
        extern_kernels.mm(reinterpret_tensor(buf301, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_119, (4096, 4096), (1, 4096), 0), out=buf305)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf306, buf308, buf290, primals_120, buf309, s0, 4096, stream=stream0)
        ...
        extern_kernels.mm(reinterpret_tensor(buf309, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_121, (4096, 14336), (1, 4096), 0), out=buf310)
        ...
        extern_kernels.mm(reinterpret_tensor(buf309, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_122, (4096, 14336), (1, 4096), 0), out=buf311)
        ...
        extern_kernels.mm(reinterpret_tensor(buf312, (s0, 14336), (14336, 1), 0), reinterpret_tensor(primals_123, (14336, 4096), (1, 14336), 0), out=buf313)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf314, buf316, buf306, primals_124, buf317, s0, 4096, stream=stream0)
        buf318 = empty_strided_cuda((s0, 4096), (4096, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_91], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf317, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_125, (4096, 4096), (1, 4096), 0), out=buf318)
        buf319 = buf296; del buf296  # reuse
        # Topologically Sorted Source Nodes: [linear_92], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf317, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_126, (4096, 1024), (1, 4096), 0), out=buf319)
        buf320 = buf295; del buf295  # reuse
        # Topologically Sorted Source Nodes: [linear_93], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf317, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_127, (4096, 1024), (1, 4096), 0), out=buf320)
        ...
        buf324 = torch.ops.aten._scaled_dot_product_efficient_attention.default(buf323, reinterpret_tensor(buf321, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf322, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf12, (1, 32, s0, s0), (s0*s0, 0, s0, 1), 0), True, scale=0.08838834764831845)
        buf325 = buf324[0]
        ...
        extern_kernels.mm(reinterpret_tensor(buf325, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_128, (4096, 4096), (1, 4096), 0), out=buf329)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf330, buf332, buf314, primals_129, buf333, s0, 4096, stream=stream0)
        ...
        extern_kernels.mm(reinterpret_tensor(buf333, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_130, (4096, 14336), (1, 4096), 0), out=buf334)
        ...
        extern_kernels.mm(reinterpret_tensor(buf333, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_131, (4096, 14336), (1, 4096), 0), out=buf335)
        ...
        extern_kernels.mm(reinterpret_tensor(buf336, (s0, 14336), (14336, 1), 0), reinterpret_tensor(primals_132, (14336, 4096), (1, 14336), 0), out=buf337)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf338, buf340, buf330, primals_133, buf341, s0, 4096, stream=stream0)
        buf342 = empty_strided_cuda((s0, 4096), (4096, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_98], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf341, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_134, (4096, 4096), (1, 4096), 0), out=buf342)
        buf343 = buf320; del buf320  # reuse
        # Topologically Sorted Source Nodes: [linear_99], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf341, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_135, (4096, 1024), (1, 4096), 0), out=buf343)
        buf344 = buf319; del buf319  # reuse
        # Topologically Sorted Source Nodes: [linear_100], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf341, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_136, (4096, 1024), (1, 4096), 0), out=buf344)
        ...
        buf348 = torch.ops.aten._scaled_dot_product_efficient_attention.default(buf347, reinterpret_tensor(buf345, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf346, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf12, (1, 32, s0, s0), (s0*s0, 0, s0, 1), 0), True, scale=0.08838834764831845)
        buf349 = buf348[0]
        ...
        extern_kernels.mm(reinterpret_tensor(buf349, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_137, (4096, 4096), (1, 4096), 0), out=buf353)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf354, buf356, buf338, primals_138, buf357, s0, 4096, stream=stream0)
        ...
        extern_kernels.mm(reinterpret_tensor(buf357, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_139, (4096, 14336), (1, 4096), 0), out=buf358)
        ...
        extern_kernels.mm(reinterpret_tensor(buf357, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_140, (4096, 14336), (1, 4096), 0), out=buf359)
        ...
        extern_kernels.mm(reinterpret_tensor(buf360, (s0, 14336), (14336, 1), 0), reinterpret_tensor(primals_141, (14336, 4096), (1, 14336), 0), out=buf361)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf362, buf364, buf354, primals_142, buf365, s0, 4096, stream=stream0)
        buf366 = empty_strided_cuda((s0, 4096), (4096, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_105], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf365, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_143, (4096, 4096), (1, 4096), 0), out=buf366)
        buf367 = buf344; del buf344  # reuse
        # Topologically Sorted Source Nodes: [linear_106], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf365, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_144, (4096, 1024), (1, 4096), 0), out=buf367)
        buf368 = buf343; del buf343  # reuse
        # Topologically Sorted Source Nodes: [linear_107], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf365, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_145, (4096, 1024), (1, 4096), 0), out=buf368)
        ...
        buf372 = torch.ops.aten._scaled_dot_product_efficient_attention.default(buf371, reinterpret_tensor(buf369, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf370, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf12, (1, 32, s0, s0), (s0*s0, 0, s0, 1), 0), True, scale=0.08838834764831845)
        buf373 = buf372[0]
        ...
        extern_kernels.mm(reinterpret_tensor(buf373, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_146, (4096, 4096), (1, 4096), 0), out=buf377)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf378, buf380, buf362, primals_147, buf381, s0, 4096, stream=stream0)
        ...
        extern_kernels.mm(reinterpret_tensor(buf381, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_148, (4096, 14336), (1, 4096), 0), out=buf382)
        ...
        extern_kernels.mm(reinterpret_tensor(buf381, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_149, (4096, 14336), (1, 4096), 0), out=buf383)
        ...
        extern_kernels.mm(reinterpret_tensor(buf384, (s0, 14336), (14336, 1), 0), reinterpret_tensor(primals_150, (14336, 4096), (1, 14336), 0), out=buf385)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf386, buf388, buf378, primals_151, buf389, s0, 4096, stream=stream0)
        buf390 = empty_strided_cuda((s0, 4096), (4096, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_112], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf389, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_152, (4096, 4096), (1, 4096), 0), out=buf390)
        buf391 = buf368; del buf368  # reuse
        # Topologically Sorted Source Nodes: [linear_113], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf389, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_153, (4096, 1024), (1, 4096), 0), out=buf391)
        buf392 = buf367; del buf367  # reuse
        # Topologically Sorted Source Nodes: [linear_114], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf389, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_154, (4096, 1024), (1, 4096), 0), out=buf392)
        ...
        buf396 = torch.ops.aten._scaled_dot_product_efficient_attention.default(buf395, reinterpret_tensor(buf393, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf394, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf12, (1, 32, s0, s0), (s0*s0, 0, s0, 1), 0), True, scale=0.08838834764831845)
        buf397 = buf396[0]
        ...
        extern_kernels.mm(reinterpret_tensor(buf397, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_155, (4096, 4096), (1, 4096), 0), out=buf401)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf402, buf404, buf386, primals_156, buf405, s0, 4096, stream=stream0)
        ...
        extern_kernels.mm(reinterpret_tensor(buf405, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_157, (4096, 14336), (1, 4096), 0), out=buf406)
        ...
        extern_kernels.mm(reinterpret_tensor(buf405, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_158, (4096, 14336), (1, 4096), 0), out=buf407)
        ...
        extern_kernels.mm(reinterpret_tensor(buf408, (s0, 14336), (14336, 1), 0), reinterpret_tensor(primals_159, (14336, 4096), (1, 14336), 0), out=buf409)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf410, buf412, buf402, primals_160, buf413, s0, 4096, stream=stream0)
        buf414 = empty_strided_cuda((s0, 4096), (4096, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_119], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf413, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_161, (4096, 4096), (1, 4096), 0), out=buf414)
        buf415 = buf392; del buf392  # reuse
        # Topologically Sorted Source Nodes: [linear_120], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf413, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_162, (4096, 1024), (1, 4096), 0), out=buf415)
        buf416 = buf391; del buf391  # reuse
        # Topologically Sorted Source Nodes: [linear_121], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf413, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_163, (4096, 1024), (1, 4096), 0), out=buf416)
        ...
        buf420 = torch.ops.aten._scaled_dot_product_efficient_attention.default(buf419, reinterpret_tensor(buf417, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf418, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf12, (1, 32, s0, s0), (s0*s0, 0, s0, 1), 0), True, scale=0.08838834764831845)
        buf421 = buf420[0]
        ...
        extern_kernels.mm(reinterpret_tensor(buf421, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_164, (4096, 4096), (1, 4096), 0), out=buf425)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf426, buf428, buf410, primals_165, buf429, s0, 4096, stream=stream0)
        ...
        extern_kernels.mm(reinterpret_tensor(buf429, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_166, (4096, 14336), (1, 4096), 0), out=buf430)
        ...
        extern_kernels.mm(reinterpret_tensor(buf429, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_167, (4096, 14336), (1, 4096), 0), out=buf431)
        ...
        extern_kernels.mm(reinterpret_tensor(buf432, (s0, 14336), (14336, 1), 0), reinterpret_tensor(primals_168, (14336, 4096), (1, 14336), 0), out=buf433)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf434, buf436, buf426, primals_169, buf437, s0, 4096, stream=stream0)
        buf438 = empty_strided_cuda((s0, 4096), (4096, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_126], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf437, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_170, (4096, 4096), (1, 4096), 0), out=buf438)
        buf439 = buf416; del buf416  # reuse
        # Topologically Sorted Source Nodes: [linear_127], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf437, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_171, (4096, 1024), (1, 4096), 0), out=buf439)
        buf440 = buf415; del buf415  # reuse
        # Topologically Sorted Source Nodes: [linear_128], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf437, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_172, (4096, 1024), (1, 4096), 0), out=buf440)
        ...
        buf444 = torch.ops.aten._scaled_dot_product_efficient_attention.default(buf443, reinterpret_tensor(buf441, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf442, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf12, (1, 32, s0, s0), (s0*s0, 0, s0, 1), 0), True, scale=0.08838834764831845)
        buf445 = buf444[0]
        ...
        extern_kernels.mm(reinterpret_tensor(buf445, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_173, (4096, 4096), (1, 4096), 0), out=buf449)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf450, buf452, buf434, primals_174, buf453, s0, 4096, stream=stream0)
        ...
        extern_kernels.mm(reinterpret_tensor(buf453, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_175, (4096, 14336), (1, 4096), 0), out=buf454)
        ...
        extern_kernels.mm(reinterpret_tensor(buf453, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_176, (4096, 14336), (1, 4096), 0), out=buf455)
        ...
        extern_kernels.mm(reinterpret_tensor(buf456, (s0, 14336), (14336, 1), 0), reinterpret_tensor(primals_177, (14336, 4096), (1, 14336), 0), out=buf457)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf458, buf460, buf450, primals_178, buf461, s0, 4096, stream=stream0)
        buf462 = empty_strided_cuda((s0, 4096), (4096, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_133], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf461, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_179, (4096, 4096), (1, 4096), 0), out=buf462)
        buf463 = buf440; del buf440  # reuse
        # Topologically Sorted Source Nodes: [linear_134], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf461, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_180, (4096, 1024), (1, 4096), 0), out=buf463)
        buf464 = buf439; del buf439  # reuse
        # Topologically Sorted Source Nodes: [linear_135], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf461, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_181, (4096, 1024), (1, 4096), 0), out=buf464)
        ...
        buf468 = torch.ops.aten._scaled_dot_product_efficient_attention.default(buf467, reinterpret_tensor(buf465, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf466, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf12, (1, 32, s0, s0), (s0*s0, 0, s0, 1), 0), True, scale=0.08838834764831845)
        buf469 = buf468[0]
        ...
        extern_kernels.mm(reinterpret_tensor(buf469, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_182, (4096, 4096), (1, 4096), 0), out=buf473)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf474, buf476, buf458, primals_183, buf477, s0, 4096, stream=stream0)
        ...
        extern_kernels.mm(reinterpret_tensor(buf477, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_184, (4096, 14336), (1, 4096), 0), out=buf478)
        ...
        extern_kernels.mm(reinterpret_tensor(buf477, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_185, (4096, 14336), (1, 4096), 0), out=buf479)
        ...
        extern_kernels.mm(reinterpret_tensor(buf480, (s0, 14336), (14336, 1), 0), reinterpret_tensor(primals_186, (14336, 4096), (1, 14336), 0), out=buf481)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf482, buf484, buf474, primals_187, buf485, s0, 4096, stream=stream0)
        buf486 = empty_strided_cuda((s0, 4096), (4096, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_140], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf485, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_188, (4096, 4096), (1, 4096), 0), out=buf486)
        buf487 = buf464; del buf464  # reuse
        # Topologically Sorted Source Nodes: [linear_141], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf485, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_189, (4096, 1024), (1, 4096), 0), out=buf487)
        buf488 = buf463; del buf463  # reuse
        # Topologically Sorted Source Nodes: [linear_142], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf485, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_190, (4096, 1024), (1, 4096), 0), out=buf488)
        ...
        buf492 = torch.ops.aten._scaled_dot_product_efficient_attention.default(buf491, reinterpret_tensor(buf489, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf490, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf12, (1, 32, s0, s0), (s0*s0, 0, s0, 1), 0), True, scale=0.08838834764831845)
        buf493 = buf492[0]
        ...
        extern_kernels.mm(reinterpret_tensor(buf493, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_191, (4096, 4096), (1, 4096), 0), out=buf497)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf498, buf500, buf482, primals_192, buf501, s0, 4096, stream=stream0)
        ...
        extern_kernels.mm(reinterpret_tensor(buf501, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_193, (4096, 14336), (1, 4096), 0), out=buf502)
        ...
        extern_kernels.mm(reinterpret_tensor(buf501, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_194, (4096, 14336), (1, 4096), 0), out=buf503)
        ...
        extern_kernels.mm(reinterpret_tensor(buf504, (s0, 14336), (14336, 1), 0), reinterpret_tensor(primals_195, (14336, 4096), (1, 14336), 0), out=buf505)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf506, buf508, buf498, primals_196, buf509, s0, 4096, stream=stream0)
        buf510 = empty_strided_cuda((s0, 4096), (4096, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_147], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf509, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_197, (4096, 4096), (1, 4096), 0), out=buf510)
        buf511 = buf488; del buf488  # reuse
        # Topologically Sorted Source Nodes: [linear_148], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf509, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_198, (4096, 1024), (1, 4096), 0), out=buf511)
        buf512 = buf487; del buf487  # reuse
        # Topologically Sorted Source Nodes: [linear_149], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf509, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_199, (4096, 1024), (1, 4096), 0), out=buf512)
        ...
        buf516 = torch.ops.aten._scaled_dot_product_efficient_attention.default(buf515, reinterpret_tensor(buf513, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf514, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf12, (1, 32, s0, s0), (s0*s0, 0, s0, 1), 0), True, scale=0.08838834764831845)
        buf517 = buf516[0]
        ...
        extern_kernels.mm(reinterpret_tensor(buf517, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_200, (4096, 4096), (1, 4096), 0), out=buf521)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf522, buf524, buf506, primals_201, buf525, s0, 4096, stream=stream0)
        ...
        extern_kernels.mm(reinterpret_tensor(buf525, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_202, (4096, 14336), (1, 4096), 0), out=buf526)
        ...
        extern_kernels.mm(reinterpret_tensor(buf525, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_203, (4096, 14336), (1, 4096), 0), out=buf527)
        ...
        extern_kernels.mm(reinterpret_tensor(buf528, (s0, 14336), (14336, 1), 0), reinterpret_tensor(primals_204, (14336, 4096), (1, 14336), 0), out=buf529)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf530, buf532, buf522, primals_205, buf533, s0, 4096, stream=stream0)
        buf534 = empty_strided_cuda((s0, 4096), (4096, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_154], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf533, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_206, (4096, 4096), (1, 4096), 0), out=buf534)
        buf535 = buf512; del buf512  # reuse
        # Topologically Sorted Source Nodes: [linear_155], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf533, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_207, (4096, 1024), (1, 4096), 0), out=buf535)
        buf536 = buf511; del buf511  # reuse
        # Topologically Sorted Source Nodes: [linear_156], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf533, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_208, (4096, 1024), (1, 4096), 0), out=buf536)
        ...
        buf540 = torch.ops.aten._scaled_dot_product_efficient_attention.default(buf539, reinterpret_tensor(buf537, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf538, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf12, (1, 32, s0, s0), (s0*s0, 0, s0, 1), 0), True, scale=0.08838834764831845)
        buf541 = buf540[0]
        ...
        extern_kernels.mm(reinterpret_tensor(buf541, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_209, (4096, 4096), (1, 4096), 0), out=buf545)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf546, buf548, buf530, primals_210, buf549, s0, 4096, stream=stream0)
        ...
        extern_kernels.mm(reinterpret_tensor(buf549, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_211, (4096, 14336), (1, 4096), 0), out=buf550)
        ...
        extern_kernels.mm(reinterpret_tensor(buf549, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_212, (4096, 14336), (1, 4096), 0), out=buf551)
        ...
        extern_kernels.mm(reinterpret_tensor(buf552, (s0, 14336), (14336, 1), 0), reinterpret_tensor(primals_213, (14336, 4096), (1, 14336), 0), out=buf553)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf554, buf556, buf546, primals_214, buf557, s0, 4096, stream=stream0)
        buf558 = empty_strided_cuda((s0, 4096), (4096, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_161], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf557, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_215, (4096, 4096), (1, 4096), 0), out=buf558)
        buf559 = buf536; del buf536  # reuse
        # Topologically Sorted Source Nodes: [linear_162], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf557, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_216, (4096, 1024), (1, 4096), 0), out=buf559)
        buf560 = buf535; del buf535  # reuse
        # Topologically Sorted Source Nodes: [linear_163], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf557, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_217, (4096, 1024), (1, 4096), 0), out=buf560)
        ...
        buf564 = torch.ops.aten._scaled_dot_product_efficient_attention.default(buf563, reinterpret_tensor(buf561, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf562, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf12, (1, 32, s0, s0), (s0*s0, 0, s0, 1), 0), True, scale=0.08838834764831845)
        buf565 = buf564[0]
        ...
        extern_kernels.mm(reinterpret_tensor(buf565, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_218, (4096, 4096), (1, 4096), 0), out=buf569)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf570, buf572, buf554, primals_219, buf573, s0, 4096, stream=stream0)
        ...
        extern_kernels.mm(reinterpret_tensor(buf573, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_220, (4096, 14336), (1, 4096), 0), out=buf574)
        ...
        extern_kernels.mm(reinterpret_tensor(buf573, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_221, (4096, 14336), (1, 4096), 0), out=buf575)
        ...
        extern_kernels.mm(reinterpret_tensor(buf576, (s0, 14336), (14336, 1), 0), reinterpret_tensor(primals_222, (14336, 4096), (1, 14336), 0), out=buf577)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf578, buf580, buf570, primals_223, buf581, s0, 4096, stream=stream0)
        buf582 = empty_strided_cuda((s0, 4096), (4096, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_168], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf581, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_224, (4096, 4096), (1, 4096), 0), out=buf582)
        buf583 = buf560; del buf560  # reuse
        # Topologically Sorted Source Nodes: [linear_169], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf581, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_225, (4096, 1024), (1, 4096), 0), out=buf583)
        buf584 = buf559; del buf559  # reuse
        # Topologically Sorted Source Nodes: [linear_170], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf581, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_226, (4096, 1024), (1, 4096), 0), out=buf584)
        ...
        buf588 = torch.ops.aten._scaled_dot_product_efficient_attention.default(buf587, reinterpret_tensor(buf585, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf586, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf12, (1, 32, s0, s0), (s0*s0, 0, s0, 1), 0), True, scale=0.08838834764831845)
        buf589 = buf588[0]
        ...
        extern_kernels.mm(reinterpret_tensor(buf589, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_227, (4096, 4096), (1, 4096), 0), out=buf593)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf594, buf596, buf578, primals_228, buf597, s0, 4096, stream=stream0)
        ...
        extern_kernels.mm(reinterpret_tensor(buf597, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_229, (4096, 14336), (1, 4096), 0), out=buf598)
        ...
        extern_kernels.mm(reinterpret_tensor(buf597, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_230, (4096, 14336), (1, 4096), 0), out=buf599)
        ...
        extern_kernels.mm(reinterpret_tensor(buf600, (s0, 14336), (14336, 1), 0), reinterpret_tensor(primals_231, (14336, 4096), (1, 14336), 0), out=buf601)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf602, buf604, buf594, primals_232, buf605, s0, 4096, stream=stream0)
        buf606 = empty_strided_cuda((s0, 4096), (4096, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_175], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf605, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_233, (4096, 4096), (1, 4096), 0), out=buf606)
        buf607 = buf584; del buf584  # reuse
        # Topologically Sorted Source Nodes: [linear_176], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf605, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_234, (4096, 1024), (1, 4096), 0), out=buf607)
        buf608 = buf583; del buf583  # reuse
        # Topologically Sorted Source Nodes: [linear_177], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf605, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_235, (4096, 1024), (1, 4096), 0), out=buf608)
        ...
        buf612 = torch.ops.aten._scaled_dot_product_efficient_attention.default(buf611, reinterpret_tensor(buf609, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf610, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf12, (1, 32, s0, s0), (s0*s0, 0, s0, 1), 0), True, scale=0.08838834764831845)
        buf613 = buf612[0]
        ...
        extern_kernels.mm(reinterpret_tensor(buf613, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_236, (4096, 4096), (1, 4096), 0), out=buf617)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf618, buf620, buf602, primals_237, buf621, s0, 4096, stream=stream0)
        ...
        extern_kernels.mm(reinterpret_tensor(buf621, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_238, (4096, 14336), (1, 4096), 0), out=buf622)
        ...
        extern_kernels.mm(reinterpret_tensor(buf621, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_239, (4096, 14336), (1, 4096), 0), out=buf623)
        ...
        extern_kernels.mm(reinterpret_tensor(buf624, (s0, 14336), (14336, 1), 0), reinterpret_tensor(primals_240, (14336, 4096), (1, 14336), 0), out=buf625)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf626, buf628, buf618, primals_241, buf629, s0, 4096, stream=stream0)
        buf630 = empty_strided_cuda((s0, 4096), (4096, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_182], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf629, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_242, (4096, 4096), (1, 4096), 0), out=buf630)
        buf631 = buf608; del buf608  # reuse
        # Topologically Sorted Source Nodes: [linear_183], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf629, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_243, (4096, 1024), (1, 4096), 0), out=buf631)
        buf632 = buf607; del buf607  # reuse
        # Topologically Sorted Source Nodes: [linear_184], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf629, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_244, (4096, 1024), (1, 4096), 0), out=buf632)
        ...
        buf636 = torch.ops.aten._scaled_dot_product_efficient_attention.default(buf635, reinterpret_tensor(buf633, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf634, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf12, (1, 32, s0, s0), (s0*s0, 0, s0, 1), 0), True, scale=0.08838834764831845)
        buf637 = buf636[0]
        ...
        extern_kernels.mm(reinterpret_tensor(buf637, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_245, (4096, 4096), (1, 4096), 0), out=buf641)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf642, buf644, buf626, primals_246, buf645, s0, 4096, stream=stream0)
        ...
        extern_kernels.mm(reinterpret_tensor(buf645, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_247, (4096, 14336), (1, 4096), 0), out=buf646)
        ...
        extern_kernels.mm(reinterpret_tensor(buf645, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_248, (4096, 14336), (1, 4096), 0), out=buf647)
        ...
        extern_kernels.mm(reinterpret_tensor(buf648, (s0, 14336), (14336, 1), 0), reinterpret_tensor(primals_249, (14336, 4096), (1, 14336), 0), out=buf649)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf650, buf652, buf642, primals_250, buf653, s0, 4096, stream=stream0)
        buf654 = empty_strided_cuda((s0, 4096), (4096, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_189], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf653, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_251, (4096, 4096), (1, 4096), 0), out=buf654)
        buf655 = buf632; del buf632  # reuse
        # Topologically Sorted Source Nodes: [linear_190], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf653, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_252, (4096, 1024), (1, 4096), 0), out=buf655)
        buf656 = buf631; del buf631  # reuse
        # Topologically Sorted Source Nodes: [linear_191], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf653, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_253, (4096, 1024), (1, 4096), 0), out=buf656)
        ...
        buf660 = torch.ops.aten._scaled_dot_product_efficient_attention.default(buf659, reinterpret_tensor(buf657, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf658, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf12, (1, 32, s0, s0), (s0*s0, 0, s0, 1), 0), True, scale=0.08838834764831845)
        buf661 = buf660[0]
        ...
        extern_kernels.mm(reinterpret_tensor(buf661, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_254, (4096, 4096), (1, 4096), 0), out=buf665)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf666, buf668, buf650, primals_255, buf669, s0, 4096, stream=stream0)
        ...
        extern_kernels.mm(reinterpret_tensor(buf669, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_256, (4096, 14336), (1, 4096), 0), out=buf670)
        ...
        extern_kernels.mm(reinterpret_tensor(buf669, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_257, (4096, 14336), (1, 4096), 0), out=buf671)
        ...
        extern_kernels.mm(reinterpret_tensor(buf672, (s0, 14336), (14336, 1), 0), reinterpret_tensor(primals_258, (14336, 4096), (1, 14336), 0), out=buf673)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf674, buf676, buf666, primals_259, buf677, s0, 4096, stream=stream0)
        buf678 = empty_strided_cuda((s0, 4096), (4096, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_196], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf677, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_260, (4096, 4096), (1, 4096), 0), out=buf678)
        buf679 = buf656; del buf656  # reuse
        # Topologically Sorted Source Nodes: [linear_197], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf677, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_261, (4096, 1024), (1, 4096), 0), out=buf679)
        buf680 = buf655; del buf655  # reuse
        # Topologically Sorted Source Nodes: [linear_198], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf677, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_262, (4096, 1024), (1, 4096), 0), out=buf680)
        ...
        buf684 = torch.ops.aten._scaled_dot_product_efficient_attention.default(buf683, reinterpret_tensor(buf681, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf682, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf12, (1, 32, s0, s0), (s0*s0, 0, s0, 1), 0), True, scale=0.08838834764831845)
        buf685 = buf684[0]
        ...
        extern_kernels.mm(reinterpret_tensor(buf685, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_263, (4096, 4096), (1, 4096), 0), out=buf689)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf690, buf692, buf674, primals_264, buf693, s0, 4096, stream=stream0)
        ...
        extern_kernels.mm(reinterpret_tensor(buf693, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_265, (4096, 14336), (1, 4096), 0), out=buf694)
        ...
        extern_kernels.mm(reinterpret_tensor(buf693, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_266, (4096, 14336), (1, 4096), 0), out=buf695)
        ...
        extern_kernels.mm(reinterpret_tensor(buf696, (s0, 14336), (14336, 1), 0), reinterpret_tensor(primals_267, (14336, 4096), (1, 14336), 0), out=buf697)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf698, buf700, buf690, primals_268, buf701, s0, 4096, stream=stream0)
        buf702 = empty_strided_cuda((s0, 4096), (4096, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_203], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf701, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_269, (4096, 4096), (1, 4096), 0), out=buf702)
        buf703 = buf680; del buf680  # reuse
        # Topologically Sorted Source Nodes: [linear_204], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf701, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_270, (4096, 1024), (1, 4096), 0), out=buf703)
        buf704 = buf679; del buf679  # reuse
        # Topologically Sorted Source Nodes: [linear_205], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf701, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_271, (4096, 1024), (1, 4096), 0), out=buf704)
        ...
        buf708 = torch.ops.aten._scaled_dot_product_efficient_attention.default(buf707, reinterpret_tensor(buf705, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf706, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf12, (1, 32, s0, s0), (s0*s0, 0, s0, 1), 0), True, scale=0.08838834764831845)
        buf709 = buf708[0]
        ...
        extern_kernels.mm(reinterpret_tensor(buf709, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_272, (4096, 4096), (1, 4096), 0), out=buf713)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf714, buf716, buf698, primals_273, buf717, s0, 4096, stream=stream0)
        ...
        extern_kernels.mm(reinterpret_tensor(buf717, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_274, (4096, 14336), (1, 4096), 0), out=buf718)
        ...
        extern_kernels.mm(reinterpret_tensor(buf717, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_275, (4096, 14336), (1, 4096), 0), out=buf719)
        ...
        extern_kernels.mm(reinterpret_tensor(buf720, (s0, 14336), (14336, 1), 0), reinterpret_tensor(primals_276, (14336, 4096), (1, 14336), 0), out=buf721)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf722, buf724, buf714, primals_277, buf725, s0, 4096, stream=stream0)
        buf726 = empty_strided_cuda((s0, 4096), (4096, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_210], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf725, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_278, (4096, 4096), (1, 4096), 0), out=buf726)
        buf727 = buf704; del buf704  # reuse
        # Topologically Sorted Source Nodes: [linear_211], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf725, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_279, (4096, 1024), (1, 4096), 0), out=buf727)
        buf728 = buf703; del buf703  # reuse
        # Topologically Sorted Source Nodes: [linear_212], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf725, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_280, (4096, 1024), (1, 4096), 0), out=buf728)
        ...
        buf732 = torch.ops.aten._scaled_dot_product_efficient_attention.default(buf731, reinterpret_tensor(buf729, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf730, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf12, (1, 32, s0, s0), (s0*s0, 0, s0, 1), 0), True, scale=0.08838834764831845)
        buf733 = buf732[0]
        ...
        extern_kernels.mm(reinterpret_tensor(buf733, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_281, (4096, 4096), (1, 4096), 0), out=buf737)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf738, buf740, buf722, primals_282, buf741, s0, 4096, stream=stream0)
        ...
        extern_kernels.mm(reinterpret_tensor(buf741, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_283, (4096, 14336), (1, 4096), 0), out=buf742)
        ...
        extern_kernels.mm(reinterpret_tensor(buf741, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_284, (4096, 14336), (1, 4096), 0), out=buf743)
        ...
        extern_kernels.mm(reinterpret_tensor(buf744, (s0, 14336), (14336, 1), 0), reinterpret_tensor(primals_285, (14336, 4096), (1, 14336), 0), out=buf745)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf746, buf748, buf738, primals_286, buf749, s0, 4096, stream=stream0)
        buf750 = empty_strided_cuda((s0, 4096), (4096, 1), torch.float16)
        # Topologically Sorted Source Nodes: [linear_217], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf749, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_287, (4096, 4096), (1, 4096), 0), out=buf750)
        buf751 = buf728; del buf728  # reuse
        # Topologically Sorted Source Nodes: [linear_218], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf749, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_288, (4096, 1024), (1, 4096), 0), out=buf751)
        buf752 = buf727; del buf727  # reuse
        # Topologically Sorted Source Nodes: [linear_219], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf749, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_289, (4096, 1024), (1, 4096), 0), out=buf752)
        ...
        buf756 = torch.ops.aten._scaled_dot_product_efficient_attention.default(buf755, reinterpret_tensor(buf753, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf754, (1, 32, s0, 128), (4096*s0, 128*s0, 128, 1), 0), reinterpret_tensor(buf12, (1, 32, s0, s0), (s0*s0, 0, s0, 1), 0), True, scale=0.08838834764831845)
        buf757 = buf756[0]
        ...
        extern_kernels.mm(reinterpret_tensor(buf757, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_290, (4096, 4096), (1, 4096), 0), out=buf761)
        ...
        stream0 = get_raw_stream(0)
        triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_9.run(buf762, buf764, buf746, primals_291, buf765, s0, 4096, stream=stream0)
        ...
        extern_kernels.mm(reinterpret_tensor(buf765, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_292, (4096, 14336), (1, 4096), 0), out=buf766)
        ...
        extern_kernels.mm(reinterpret_tensor(buf765, (s0, 4096), (4096, 1), 0), reinterpret_tensor(primals_293, (4096, 14336), (1, 4096), 0), out=buf767)
        ...
        extern_kernels.mm(reinterpret_tensor(buf768, (s0, 14336), (14336, 1), 0), reinterpret_tensor(primals_294, (14336, 4096), (1, 14336), 0), out=buf769)
...
def benchmark_compiled_module(times=10, repeat=10):
    ...
    primals_4 = 56
    primals_5 = rand_strided((1, 56), (56, 1), device='cuda:0', dtype=torch.int64)
    primals_6 = rand_strided((64, ), (1, ), device='cuda:0', dtype=torch.float32)
    primals_7 = rand_strided((4096, ), (1, ), device='cuda:0', dtype=torch.float16)
    primals_8 = rand_strided((4096, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_9 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_10 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_11 = rand_strided((4096, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_14 = rand_strided((14336, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_15 = rand_strided((4096, 14336), (14336, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_19 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_20 = rand_strided((4096, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_23 = rand_strided((14336, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_24 = rand_strided((4096, 14336), (14336, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_28 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_32 = rand_strided((14336, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_33 = rand_strided((4096, 14336), (14336, 1), device='cuda:0', dtype=torch.float16)
    primals_34 = rand_strided((4096, ), (1, ), device='cuda:0', dtype=torch.float16)
    primals_35 = rand_strided((4096, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_36 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_37 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_41 = rand_strided((14336, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_42 = rand_strided((4096, 14336), (14336, 1), device='cuda:0', dtype=torch.float16)
    primals_43 = rand_strided((4096, ), (1, ), device='cuda:0', dtype=torch.float16)
    primals_44 = rand_strided((4096, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_45 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_46 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_47 = rand_strided((4096, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_50 = rand_strided((14336, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_51 = rand_strided((4096, 14336), (14336, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_55 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_56 = rand_strided((4096, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_59 = rand_strided((14336, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_60 = rand_strided((4096, 14336), (14336, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_64 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_68 = rand_strided((14336, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_69 = rand_strided((4096, 14336), (14336, 1), device='cuda:0', dtype=torch.float16)
    primals_70 = rand_strided((4096, ), (1, ), device='cuda:0', dtype=torch.float16)
    primals_71 = rand_strided((4096, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_72 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_73 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_77 = rand_strided((14336, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_78 = rand_strided((4096, 14336), (14336, 1), device='cuda:0', dtype=torch.float16)
    primals_79 = rand_strided((4096, ), (1, ), device='cuda:0', dtype=torch.float16)
    primals_80 = rand_strided((4096, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_81 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_82 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_83 = rand_strided((4096, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_86 = rand_strided((14336, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_87 = rand_strided((4096, 14336), (14336, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_91 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_92 = rand_strided((4096, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_95 = rand_strided((14336, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_96 = rand_strided((4096, 14336), (14336, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_100 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_104 = rand_strided((14336, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_105 = rand_strided((4096, 14336), (14336, 1), device='cuda:0', dtype=torch.float16)
    primals_106 = rand_strided((4096, ), (1, ), device='cuda:0', dtype=torch.float16)
    primals_107 = rand_strided((4096, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_108 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_109 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_113 = rand_strided((14336, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_114 = rand_strided((4096, 14336), (14336, 1), device='cuda:0', dtype=torch.float16)
    primals_115 = rand_strided((4096, ), (1, ), device='cuda:0', dtype=torch.float16)
    primals_116 = rand_strided((4096, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_117 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_118 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_119 = rand_strided((4096, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_122 = rand_strided((14336, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_123 = rand_strided((4096, 14336), (14336, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_127 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_128 = rand_strided((4096, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_131 = rand_strided((14336, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_132 = rand_strided((4096, 14336), (14336, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_136 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_140 = rand_strided((14336, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_141 = rand_strided((4096, 14336), (14336, 1), device='cuda:0', dtype=torch.float16)
    primals_142 = rand_strided((4096, ), (1, ), device='cuda:0', dtype=torch.float16)
    primals_143 = rand_strided((4096, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_144 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_145 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_149 = rand_strided((14336, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_150 = rand_strided((4096, 14336), (14336, 1), device='cuda:0', dtype=torch.float16)
    primals_151 = rand_strided((4096, ), (1, ), device='cuda:0', dtype=torch.float16)
    primals_152 = rand_strided((4096, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_153 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_154 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_155 = rand_strided((4096, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_158 = rand_strided((14336, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_159 = rand_strided((4096, 14336), (14336, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_163 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_164 = rand_strided((4096, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_167 = rand_strided((14336, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_168 = rand_strided((4096, 14336), (14336, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_172 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_176 = rand_strided((14336, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_177 = rand_strided((4096, 14336), (14336, 1), device='cuda:0', dtype=torch.float16)
    primals_178 = rand_strided((4096, ), (1, ), device='cuda:0', dtype=torch.float16)
    primals_179 = rand_strided((4096, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_180 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_181 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_185 = rand_strided((14336, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_186 = rand_strided((4096, 14336), (14336, 1), device='cuda:0', dtype=torch.float16)
    primals_187 = rand_strided((4096, ), (1, ), device='cuda:0', dtype=torch.float16)
    primals_188 = rand_strided((4096, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_189 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_190 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_191 = rand_strided((4096, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_194 = rand_strided((14336, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_195 = rand_strided((4096, 14336), (14336, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_199 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_200 = rand_strided((4096, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_203 = rand_strided((14336, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_204 = rand_strided((4096, 14336), (14336, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_208 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_212 = rand_strided((14336, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_213 = rand_strided((4096, 14336), (14336, 1), device='cuda:0', dtype=torch.float16)
    primals_214 = rand_strided((4096, ), (1, ), device='cuda:0', dtype=torch.float16)
    primals_215 = rand_strided((4096, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_216 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_217 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_221 = rand_strided((14336, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_222 = rand_strided((4096, 14336), (14336, 1), device='cuda:0', dtype=torch.float16)
    primals_223 = rand_strided((4096, ), (1, ), device='cuda:0', dtype=torch.float16)
    primals_224 = rand_strided((4096, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_225 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_226 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_227 = rand_strided((4096, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_230 = rand_strided((14336, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_231 = rand_strided((4096, 14336), (14336, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_235 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_236 = rand_strided((4096, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_239 = rand_strided((14336, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_240 = rand_strided((4096, 14336), (14336, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_244 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_248 = rand_strided((14336, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_249 = rand_strided((4096, 14336), (14336, 1), device='cuda:0', dtype=torch.float16)
    primals_250 = rand_strided((4096, ), (1, ), device='cuda:0', dtype=torch.float16)
    primals_251 = rand_strided((4096, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_252 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_253 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_257 = rand_strided((14336, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_258 = rand_strided((4096, 14336), (14336, 1), device='cuda:0', dtype=torch.float16)
    primals_259 = rand_strided((4096, ), (1, ), device='cuda:0', dtype=torch.float16)
    primals_260 = rand_strided((4096, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_261 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_262 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_263 = rand_strided((4096, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_266 = rand_strided((14336, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_267 = rand_strided((4096, 14336), (14336, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_271 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_272 = rand_strided((4096, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_275 = rand_strided((14336, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_276 = rand_strided((4096, 14336), (14336, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_280 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_284 = rand_strided((14336, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_285 = rand_strided((4096, 14336), (14336, 1), device='cuda:0', dtype=torch.float16)
    primals_286 = rand_strided((4096, ), (1, ), device='cuda:0', dtype=torch.float16)
    primals_287 = rand_strided((4096, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_288 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_289 = rand_strided((1024, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    ...
    primals_293 = rand_strided((14336, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
    primals_294 = rand_strided((4096, 14336), (14336, 1), device='cuda:0', dtype=torch.float16)
    primals_295 = rand_strided((4096, ), (1, ), device='cuda:0', dtype=torch.float16)
    primals_296 = rand_strided((128256, 4096), (4096, 1), device='cuda:0', dtype=torch.float16)
